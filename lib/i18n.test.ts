...
test('test1', async () => {
    ...
    const xlated = t('test.foo');
    expect(xlated).toBe("DE-bar");
});

test('test2', async () => {
    const i18n = new I18N("en-US");
    const t = i18n.getTranslator(); const xlated = t('test.foo');
    expect(xlated).toBe("US-bar");
});

test('testFallback', async () => {
    const i18n = new I18N("de");
    const t = i18n.getTranslator(); const xlated = t('test.fallback');
    expect(xlated).toBe("US-fallback");
});

test('testParams', async () => {
    const i18n = new I18N("en-US");
    const t = i18n.getTranslator(); const xlated = t('test.baz', { "param1": "p1", "param2": "p2" });
    expect(xlated).toBe("US-bang p1, p2");
});

test('testNoLocale', async () => {
