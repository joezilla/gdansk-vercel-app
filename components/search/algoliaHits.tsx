  ...
  return (
    hit.images && hit.images.length > 0
      ...
      <img src={hit.images[0].url}
        alt={hit.images[0].title} className="object-cover h-40 object-center" />
      :
      <img src="/images/No-Image-Placeholder.png"
        alt="no picture" className="object-cover h-40 object-center" />
  );
}
...
function AlgoliaHitRenderer({ searchState, searchResults }: { searchState: any, searchResults: any }) {
  ...
  return (
    <>
      ...
      {searchResults?.hits.length > 0 && validQuery && (
        <div className="mt-6 grid grid-cols-1 gap-y-4 gap-x-6 grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
