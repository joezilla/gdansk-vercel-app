    ...
    <header className="p-4 dark:bg-gray-800 dark:text-gray-100 sticky top-0">
      ...
      */}
        ...
        <div className="md:invisible p-4">
          <Menu left pageWrapId={"page-wrap"} outerContainerId={"outer-container"} width="50%">
            ...
            {navigationPosts.navigationPosts.map(post =>
              <a id={post.slug} key={post.slug} className="menu-item dark:text-white" href={`/posts/${post.slug}`}>{post.title}</a>
            )}
          </Menu>
        </div>
        {/*}
